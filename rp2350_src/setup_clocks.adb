...
with Interfaces.RP2350.CLOCKS;   use Interfaces.RP2350.CLOCKS;
with Interfaces.RP2350.PLL_SYS;  use Interfaces.RP2350.PLL_SYS;
with Interfaces.RP2350.RESETS;   use Interfaces.RP2350.RESETS;
with Interfaces.RP2350.ROSC;     use Interfaces.RP2350.ROSC;
with Interfaces.RP2350.XOSC;     use Interfaces.RP2350.XOSC;

with System.BB.Board_Parameters; use System.BB.Board_Parameters;

with RP2350_Runtime_Config; use RP2350_Runtime_Config;
...
procedure Setup_Clocks is
   ...
   function SYS_CLK_SEL_Mask (SRC : CLK_SYS_CTRL_SRC_Field) return UInt2
   is (UInt2 (Shift_Left
                (Byte (1), Natural (CLK_SYS_CTRL_SRC_Field'Pos (SRC)))));

   function REF_CLK_SEL_Mask (SRC : CLK_REF_CTRL_SRC_Field) return UInt4
   is (UInt4 (Shift_Left
                (Byte (1), Natural (CLK_REF_CTRL_SRC_Field'Pos (SRC)))));

   procedure Enable_XOSC;
   procedure Enable_ROSC;

   procedure Enable_PLL
      (PLL_Periph    : not null access PLL_SYS_Peripheral;
       Reference     : Hertz;
   ...
   procedure Enable_XOSC is
      ...
      pragma Compile_Time_Error
        (System.BB.Board_Parameters.XOSC_Startup_Delay_Mult = 0,
         "XOSC_Startup_Delay_Mult cannot be zero");

      pragma Compile_Time_Error
        (System.BB.Board_Parameters.XOSC_Startup_Delay_Mult
         > (STARTUP_DELAY_Field'Last / Startup_Delay),
   ...
   begin
      ...
      XOSC_Periph.STARTUP.DELAY_k :=
        Startup_Delay * System.BB.Board_Parameters.XOSC_Startup_Delay_Mult;
   ...
   begin
      ...
      --  Turn on PLL
      PLL_Periph.PWR.PD    := 0;
      PLL_Periph.PWR.VCOPD := 0;

      --  Wait for lock
      while PLL_Periph.CS.LOCK = 0 loop
         null;
      end loop;
      ...
      PLL_Periph.PWR.POSTDIVPD := 0;
   end Enable_PLL;

   --  The SVD generates different types for PLL_SYS and PLL_USB, even though
   --  they both have the same register layout. So we define our own
   --  PLL_USB_Periph here uses the PLL_SYS registers so we can reuse code.

   PLL_USB_Periph : aliased PLL_SYS_Peripheral with
     Import, Address => PLL_USB_Base;
...
begin
   ...
   while CLOCKS_Periph.CLK_SYS_SELECTED.CLK_SYS_SELECTED
         /= SYS_CLK_SEL_Mask (clk_ref)
   loop
      null;
   end loop;
   ...
   if Has_XOSC then
      ...
      while CLOCKS_Periph.CLK_REF_SELECTED.CLK_REF_SELECTED
