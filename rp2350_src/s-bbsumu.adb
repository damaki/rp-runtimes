...
with Interfaces.RP2350;
with Interfaces.RP2350.PSM; use Interfaces.RP2350.PSM;
with Interfaces.RP2350.SIO; use Interfaces.RP2350.SIO;
...
package body Multiprocessors is
   ...
   function Number_Of_CPUs return CPU is (RP2350_Runtime_Config.Max_CPUs);
   ...
   begin
      if BB.Parameters.Multiprocessor then
         ...
         Enable_Interrupt_Request (Doorbell_IRQ, Interrupt_Priority'First);
      end if;
   end Start_All_CPUs;
   ...
   begin
      ...
      --  Enable interrupts
      NVIC_ISER0 := Word (ISER_Core1 and 16#FFFF_FFFF#);
      NVIC_ISER1 := Word (Shift_Right (ISER_Core1, 32));

      --  Enable doorbell interrupt to receive pokes from core0.
      ...
      NVIC_ISPR0 := 2**Natural (Doorbell_IRQ);
